      ...
      <div className="card" data-name="compliance-status" data-file="components/ComplianceStatus.js">
        ...
        {isAdmin ? (
            ...
            {complianceData.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between p-3 bg-[var(--bg-light)] rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.status === 'compliant'
          ...
          <div className="space-y-4">
            <div className={`p-4 rounded-lg border-2 ${complianceData.status === 'compliant'
              ...
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${complianceData.status === 'compliant' ? 'bg-green-500' : 'bg-red-500'
                  }`}>
                  <div className={`text-xl text-white ${complianceData.status === 'compliant' ? 'icon-check' : 'icon-x'
                    }`}></div>
                </div>
                <div>
